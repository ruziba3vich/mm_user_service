...
type (
	UserRepo interface {
		...
		CheckUserAFollowsUserB(ctx context.Context, userA, userB string) (bool, error)
		FollowUserBByUserA(ctx context.Context, userA, userB, generatedID string) error
		...
		GetFollowers(ctx context.Context, userID string, page, limit int32) ([]*models.User, int32, error)
	}
)
